 ...
 *
 * Nothing in the transpiler asks for this - it is esbuild's own behaviour - which is exactly why it is
 * worth pinning. The failure it prevents is silent: a bundler that treats an unresolvable bare
 * specifier as external drops the import statement, leaves the identifier it bound as a free name, and
 * the emitted mod file carries a symbol the game cannot resolve:
 *
 *     SetGlobal("unresolved", "GLOBAL", Missing)
 *
 * That is a miscompile with no error anywhere, and whether it happens is a property of the bundler
 * rather than of anything in this repo. The only imports legitimately left unresolved are `.d.ts`
 * declarations, which the resolver plugins mark external by name rather than by falling through.
 */
