...
6. **No work-time artifacts in the repo.** Exception: `tmp/` (in `.gitignore`).
   Rules 7–9 describe the named-projection architecture the canonical-doc layer is moving toward. Flag fields are migrated end-to-end across all five formats (PRO / MAP / ITM / SPL / EFF) — the wire codec produces the named-bit dict shape, the canonical zod validates it, and reservoirs preserve unnamed bits. Enum and PID projections (named string with int fallback for enums; tagged composite for PIDs) are not yet wired; canonical-doc surfaces those as raw integers today. Helpers in `binary/src/spec/coded-projection.ts` (`intToEnumValue` / `enumValueToInt`) exist for hand-written canonical schemas that opt in. See `docs/todo.md` for the remaining migration work.

7. **Named projection of coded scalars.** Where the wire encodes a _set_ of named bits (flag word), a _coded value_ drawn from a fixed table (enum), or a _composite_ of independent semantic units (PID = type + id), the canonical-doc surfaces the semantic shape, not the raw integer. Flag word `u32` projects to `{[name]: boolean, _bits: hexString}`; an enum int projects to a string-name when the value is in the table, otherwise stays as a `number` (`string | number` zod union); a composite PID projects to a tagged object (`{type: name, id}` when type is known, `{typeRaw: int, id}` otherwise). The wire codec packs/unpacks at the byte boundary; everything above sees the named shape. This is a consistent application of rule #1 — `packedAs`+`bitRange` already does this for byte-packed fields, the canonical layer exposes the semantic units the wire conflates. Same shape applies to flat-with-absolute-indices arrays whose semantic meaning is hierarchical (ITM equipping vs per-ability effects pointing into one flat `effects[]` pool by absolute index): canonical projects nested, the wire codec flattens on serialize and re-slices on parse.

8. **Lossless reservoirs preserve unnamed data.** Every named projection is paired with a reservoir at the same granularity. Bit-level: `_bits` (hex string) carries unnamed bits in a flag word, default `"0x0"` / absent when all bits are named/zero. Strict-disjoint with the named bits — a hand-edit putting a named bit in `_bits` fails to load. Value-level: enum `string | number` union and PID `{type, id}` vs `{typeRaw, id}` shape distinction preserve unknown values; the strict gate (rule #3) rejects unknown ints only for closed enums (`enumOpen` not set). Region-level: `opaqueRanges` (existing). The byte round-trip invariant `serialize(parse(b)) === b` is the load-bearing property of this model — every existing `*-roundtrip.test.ts` already enforces it.

9. **Spec is the active interpreter; the snapshot is the byte-faithful record.** Adding a name to a flag/enum table is a non-breaking spec evolution: old snapshots load via the reservoir/union path, re-saving promotes the bits/values to the new names. `schemaVersion` does NOT bump for additive name changes; bumping is reserved for _re-interpretive_ spec changes (a previously-parsed field's meaning changes), which require explicit migration code in the snapshot codec. Reservoir-substitutive changes (a previously-opaque region gains structure) accept both shapes on load with no version bump and re-save with the new shape.

The named-projection rules are also the foundation for the construction API recorded in `docs/todo.md` (per-format wrapper classes with `new Item({...})` constructors and `.abilities.add(...)` collection helpers). The named-dict canonical shape _is_ the construction-time API surface, with no translation layer between them; default-value derivation (`spec/derive-default.ts`) is the single source of truth for per-spec defaults so the construction API doesn't re-derive them.
