...
| `--graceful-map`    | Permissive boundary guessing for ambiguous MAP files. Not needed on `--load`: a snapshot carrying an `objects-tail` opaque range reloads with permissive boundaries on its own |
