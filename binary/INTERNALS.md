...
- **Structural edits** (optional, implemented but intentionally dormant): `isStructuralFieldId`, `buildStructuralTransitionBytes` - layout-changing edits (a PRO object-type/subtype change rewrites the whole record). PRO implements both, but no runtime path invokes them: `objectType`/`subType` are `readOnlyFields` (changing a proto's type is a destructive whole-record retype the editor does not offer), and `editField` does a plain value mutation without consulting `isStructuralFieldId`. The methods are kept as the scaffolding for a future explicit "retype" affordance; until one is wired, the capability never fires.
...
- **Every structural mutation rebuilds bytes, then reparses the whole file.** A structure op builds new bytes via the adapter byte-builder, and `binary-editor` reparses them to rebuild the model (`binary-editor/src/structure-ops.ts`). This is `O(file)` per op but keeps the wire format the single source of truth - the post-op model is exactly what a fresh open of the resulting bytes would produce, so there is no separate "mutate the in-memory tree" path to keep consistent with the serializer. It is correctness-preserving and fast enough at PRO/ITM/SPL/CRE scale; MAP leans on `skipMapTiles` so the reparse skips the ~40k-tile bulk. Keep it; do not add an in-place tree-mutation fast path without a measured need.
- **The byte-builders live in `binary/` (this package), not in the editor.** Producing the inserted/removed/reordered bytes requires the format's codec and canonical writer, which live here. This is a deliberate, narrow widening of the parser layer's role ("the format package also knows how to mutate its own structure"), the same layer-spanning choice as the canonical readers. The editor stays format-agnostic: it resolves the structural target (section path + ordinal) and calls the adapter.
- **`BinaryFormatAdapter` intentionally spans data and presentation concerns.** It carries data concerns (snapshot, canonical rebuild, domain ranges, byte-builders) alongside presentation/structure predicates consumed only by the editor's layout (`isListSection` / `isModifiableArray` / `isAddableArray` / `isRemovableEntry`). This is single-source-of-truth: the alternative is a parallel "which sections are lists" table in `binary-editor`. The predicates are pure, path-based, and format-knowledge-driven - keep data logic out of them. The declarative layout schema (where a format opts into it) is the same kind of presentation-only data attached to the adapter.
...
3. **Read permissive, write strict.** Out-of-range enum values display as `Unknown (N)` and parse succeeds; saving rejects via the zod refinement when `spec.enum` is set. Real-world exception: MAP object base `rotation`/`elevation` carry packed-PID-shaped values in shipped files - the canonical zod stays plain int32 for those even though the wire spec documents enum tables.
   ...
   **Closed vs open enums.** The strict gate fits enums whose value space is fixed by the engine AND fully vendored (PRO `objectType`: Item/Critter/Scenery/Wall/Tile/Misc - adding a 7th would crash the engine). Set `enumOpen: true` wherever the vendored table is not the whole story - either the value space is genuinely extensible (ITM type, mod-extensible via `itemtype.2da`), the engine tolerates out-of-table values (ITM ability `damageType` / `projectileType`, SPL `type` and `castingGraphics`), or the table is narrower than the engines are. IE effect opcodes are the last case: the opcode set is FIXED - mods do not add opcodes - but a number's meaning is per-engine and the shipped readings do not cover every engine, so a strict gate would refuse to save a file that is legal in its own engine. The display lookup still resolves named values; the strict refinement does not enforce membership. Closed-default keeps PRO/MAP behaviour unchanged; opt-in keeps mod-friendly fields editable without producing false rejections at save time.
...
| Per-format scaffolding (canonical reader/writer/snapshot/serializer/format-adapter wrappers for ITM, SPL, EFF, CRE) | factored via `ie-common/canonical-reader.ts` + `ie-common/json-snapshot.ts` + `ie-common/serializer.ts` | The canonical-reader/JSON-snapshot/serializer bodies were byte-identical across the four formats once N reached >=3; only the schema types and the format-id discriminant varied. Per-format files now delegate to the factory and supply just the typed pieces. The format-id literal union (`"itm" \| "spl" \| "eff" \| "cre"`) caps drift - adding a fifth IE format means widening the union and writing the new format's specs, not duplicating the wrapper boilerplate |
...
2. **Parser.** `index.ts` implements `BinaryParser` (`id`, `name`, `extensions`, `family`, `parse`, optional `serialize`). `family` is load-bearing, not metadata: the two game families collide on `.pro`, so a consumer that knows which game its file came from looks the parser up by extension AND family, and the VS Code client's IE resource browser derives which formats open in the binary editor from it. Tag a new parser correctly and it routes itself. Use `toTypedBinarySchema(spec)` for wire reads, `walkStruct(spec, presentation, ...)` for the display tree, and orchestrate any subtype dispatch / recursion in the parser itself.
...
- **No cross-record consistency check for MAP (by design - its candidate relations aren't validatable).** The CRE/ITM/SPL cross-record diagnostics (dangling intra-file references surfaced as warnings via `binary-editor`'s relationship `constraints`) have no MAP analogue, even though MAP appears to relate objects to scripts. All three candidate relations were investigated against fallout2-ce source and the 6-map fixture corpus and ruled out: (1) object `scriptIndex` is a dead engine runtime cache (`// TODO: remove` on the engine's `Object` struct, `src/obj_types.h`), not a position into the script table - the engine binds object to script by `sid` alone (`scriptGetScript`, `src/scripts.cc`), and real maps store non-positional values there (e.g. 511/750/1473 in artemple with a handful of scripts). (2) An object `sid` with no matching map script slot is NOT corruption: at load `objectLoadAllInternal` silently resets the unresolved sid to -1 (`src/object.cc`), and objects can receive scripts from their proto via `objectSetScriptFromProto` independent of the map's local script lists - so a sid-membership check would warn on data the engine treats as routine. (Corpus check: unmatched object sids occur only in maps that already fail to fully parse - an `objects-tail` opaque range - i.e. parser artifacts, not real dangling refs.) (3) Orphan-script detection is undecidable: spatial (type 1) and timer (type 2) scripts are legitimately ownerless. See the `sid` / `scriptIndex` field comments in `binary/src/map/specs/object.ts` for the engine-sourced detail. Do not add a MAP cross-record check.
