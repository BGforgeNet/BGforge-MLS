...
7. **Named-bit dict projection for flag fields.** A flag-word spec entry (`{codec, flags: Table}`) surfaces in canonical-doc as a `{[name]: boolean, _bits?: hexString}` strict-object dict, not as the raw int. Toggling one bit changes one line in the snapshot diff; toggling many bits changes one line per bit. Each named bit is required (the dict is exhaustive over the table); `_bits` is an optional hex-string reservoir for unnamed bits in the wire word. Strict-disjoint invariant: named bits never appear in `_bits`, enforced at the wire boundary by `flagDictToInt`. `compileFlagTable` slugifies the table's display strings to camelCase canonical keys (`"NoBlock"` → `noBlock`); `intToFlagDict` / `flagDictToInt` translate at the wire codec boundary via the `FlagDictSchema` wrapper.

    This is a consistent application of rule #1 — `packedAs`+`bitRange` already exposes byte-packed sub-fields as peer scalar entries; the named-bit dict exposes bit-packed sub-fields the same way (the wire packs N independent semantic units into one int; canonical separates them).

8. **Lossless reservoirs preserve unnamed bits.** Adding a name to a flag table is a non-breaking spec evolution: old snapshots load via the `_bits` path, re-saving promotes the bit to its new name. `schemaVersion` does NOT bump for additive name changes; bumping is reserved for _re-interpretive_ spec changes (a previously-parsed field's meaning changes), which require explicit migration code in the snapshot codec. The byte round-trip invariant `serialize(parse(b)) === b` is the load-bearing property — every existing `*-roundtrip.test.ts` enforces it.

9. **Enums and PIDs stay numeric in canonical-doc by design.** Where flag fields project to dicts (rule #7), enum and PID fields stay as raw integers — the diff-friendliness gain doesn't justify the complication. Half the enum fields drive dispatch (`objectType`, `subType`, `scriptType`, MAP `version` / `rotation` / `elevation`) and would force a conversion at every dispatch site if projected to strings; the rest produce diffs of the same line count whether named or numeric (`5 → 0` vs `"Items" → "Background"`), unlike flags where the diff is fundamentally lossy. The display layer's `enum` table resolves names for editor dropdowns and hover; the snapshot stays close to the wire. Helpers `intToEnumValue` / `enumValueToInt` in `coded-projection.ts` exist as opt-in utilities for code that prefers named-string ergonomics (e.g. the construction API in `docs/todo.md`); they're not wired through the derivation primitives.
