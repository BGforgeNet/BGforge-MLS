...
- **PRO-resolved item/scenery payload decode.** `parseObjectAt` returns `"TODO opaque"` for item/scenery objects whose payload layout depends on PRO-resolved subtype information. Closing this needs a PRO metadata resolver that doesn't exist yet — separate work, not a refactor task.
