...
- `server/src/<grammar>/tree-sitter.d.ts` - the node-type declarations (types only). **Not tracked**
  (gitignored uniformly by the `server/src/*/tree-sitter.d.ts` pattern): fully regenerated by
  `build:grammar`, which runs before every typecheck in CI, so a fresh clone typechecks after that step.
