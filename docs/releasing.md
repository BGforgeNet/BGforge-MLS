...
`bgforge-mls-bundles` CI artifact beside the editor bundles - the same `dist/*.zip` glob. That is where an
unreleased grammar fix is available from; releases remain the only published channel. The SBOM is generated on
release tags only, so no CI artifact carries one.

## Pre-release checklist

Walk this before tagging a `vX.Y.Z` extension release. The detailed procedures are in the sections below; this is the preflight that catches what is easy to forget. The extension ships regardless of the npm library versions (see _The server and VSIX bundle their libraries_ below), so nothing forces a library or Action release alongside it: a component that changed since its own last tag must be released deliberately here, or it ships stale to its own consumers while the extension moves on.
...
- [ ] **Changelog finalized.** Rename the `## Unreleased` heading in `docs/changelog.md` to the new version, and confirm every user-facing change since the last tag is listed - features, bug fixes, and behavior changes alike (`git log --oneline <last-tag>..HEAD` for the source set). Implementation-only commits (refactors, tests, CI, build) stay out, and so do reusable Action changes, which go in `actions/<name>/CHANGELOG.md` (_Releasing a reusable Action_ below).
- [ ] **Independently-versioned libraries checked.** `@bgforge/binary`, `@bgforge/format`, `@bgforge/transpile`, and `@bgforge/tssl` version on their own tags; their npm packages and CLIs (`fgbin`, `fgfmt`, `fgtp`, `tssl`) update only when you tag them. If `binary/src`, `format/src`, `transpilers/src`, or `compilers/tssl/src` carries unreleased changes you want on npm (`pnpm view @bgforge/<pkg> version` vs the `package.json`), bump and tag each (`binary/vX.Y.Z`, `format/vX.Y.Z`, `transpile/vX.Y.Z`, `tssl/vX.Y.Z`) - see _Releasing a library_ below.
- [ ] **Reusable Actions checked.** `actions/binary`, `actions/format`, `actions/transpile`, and `actions/tssl` are consumed at a pinned git ref, so a consumer on `actions/<name>/v1` sees a change to the ACTION only when you move that alias. Add its changelog section, cut a new `actions/<name>/vX.Y.Z` and re-point `actions/<name>/v1` when `actions/**` has changed since that action's last tag - its own `action.yml` or the shared `actions/_shared/` scripts. A CLI change alone is NOT a reason - publish the library instead (the item above); why is in _Releasing a reusable Action_ below.
- [ ] **Security advisories cleared.** Run `pnpm audit`, and review the repository's open **Dependabot security alerts** (Security tab; which Dependabot channels are on is in [supply-chain.md](supply-chain.md)) and the **CodeQL** / **OpenSSF Scorecard** code-scanning findings. A new high-severity advisory is a release blocker, not a note-for-later: resolve each before tagging - bump the dependency, add a `pnpm` override pin, or, when the vulnerable path is genuinely unreachable, dismiss it with a recorded rationale. See [dependencies.md](dependencies.md).
- [ ] **Pins and overrides still necessary.** Each version pin and override is a workaround for a specific upstream state, not a permanent fixture. Walk every entry in [dependencies.md](dependencies.md) and the `overrides` + `minimumReleaseAgeExclude` in `pnpm-workspace.yaml`. For each deliberate hold, check whether its "revisit when X" trigger is now met (a matured major shipped, an advisory closed, an upstream patched its bundled dep so the override is redundant); for each override, test removing it (`pnpm install` + the gate) and keep only the minimal set still required. Drop what is no longer needed and keep the recorded rationale current for what stays.
- [ ] **Release secrets valid.** The tag workflow publishes with the `VS_MARKETPLACE_TOKEN`, `OPEN_VSX_TOKEN` and `NPM_TOKEN` repository secrets; an expired token fails its publish step after the GitHub Release already exists (see _Recovering from a partial release_ below).
...
- [ ] **E2E green.** `pnpm test:e2e` passes (after `pnpm build`). `test:all` excludes it and CI runs it only on release tags, so otherwise the tag push is its first run.
...
Neither the published `@bgforge/mls-server` nor the VSIX declares an `@bgforge/*` package as a runtime dependency. The server bundles `@bgforge/format` and the transpilers into `server/out/server.js`, and the VSIX bundles `@bgforge/binary` and `@bgforge/binary-editor` into the client bundle (esbuild externalizes only `vscode` and `esbuild-wasm`); `@bgforge/format` is a `devDependency` of `server/`, consumed at build time. So a fresh `npm install @bgforge/mls-server` resolves no `@bgforge/*` packages, and the extension and the libraries release in any order. A library bump is needed only to publish that library's own npm package and CLI for external consumers.
...
it - see the `pnpm` entry in [dependencies.md](dependencies.md).
...
An Action's tag covers the Action, not the CLI it drives. Each installs its CLI from npm rather than building the repo at the pinned ref - `npm install -g "@bgforge/<pkg>@${PKG_VERSION}"`, with `PKG_VERSION` taken from the caller's `version` input (default `latest`) - so the tag pins the workflow steps while the CLI version is resolved at run time. A library release reaches Action consumers the moment it is published, re-tagging the Action for a CLI change would ship nothing, and only a change under `actions/**` calls for a new Action tag.

1. Add a `## X.Y.Z` section to `actions/<name>/CHANGELOG.md` covering the Action's consumer-facing changes since its last tag - its inputs, outputs, and what a run commits or checks. Changes to the CLI it installs belong in that library's changelog, not here.
2. Choose the commit to release. It must contain the `actions/<name>/` directory (changelog included), the shared `actions/_shared/` directory it sources at runtime, and the current, filtered `build.yml`. A commit from before the trigger was filtered is unsafe: its `build.yml` runs on every tag and would start a full extension release on the Action tag push.
3. Create the immutable tag and move the major alias to it (example for `binary`; substitute `format`, `transpile` or `tssl`):
   ```
   ...
   ```
4. Bump the major (`v2`) for breaking changes to that Action's interface. The four Actions version independently - a bump to one does not move the others.
