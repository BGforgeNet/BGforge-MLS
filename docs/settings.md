...
| `bgforge.binaryEditor.autoDumpJson` | `false`       | VS Code only - the language server does not read it. Whether saving a file in the binary editor should also write a JSON snapshot next to it.                                                                                                                                                                 |
| `bgforge.debug`                     | `false`       | Enable debug logging in the Output panel (BGforge MLS channel). On switching on, logs the editor's name and the LSP capabilities its client announced.                                                                                                                                                        |
...
| `bgforge.weidu.tlkEncoding` | `""`    | Character encoding of a **classic** game's `dialog.tlk`. Empty means UTF-8 for Enhanced Editions and windows-1252 otherwise, which covers every EE install and the Western classic languages. Set it only when game text shows as garbled characters: `windows-1250` (Czech, Polish, Hungarian), `windows-1251` (Russian, Ukrainian), `windows-1254` (Turkish), `shift_jis`, `gbk`, `euc-kr`, `big5`. A classic install records its encoding nowhere, so it cannot be detected.                                                                             |
| `bgforge.weidu.compiler`    | `weidu` | Which compiler produces BAF diagnostics. `weidu` is the WeiDU binary at `bgforge.weidu.path`, the reference: it resolves the `%variable%` substitutions and translation references that only exist once a mod is being installed. `built-in` is the extension's own; it needs no WeiDU binary, compiles the editor's text directly, and reports every problem at once - but refuses a file using `%variable%` or a `@123` translation reference, naming the construct, since those values only exist during an install. Both need `bgforge.weidu.gamePath`. |
...
- [Geany](editors/geany.md#settings) - `initialization_options` in LSP Client config, which the plugin serves to
