...
- A creature's embedded v2 effects now open with the same detailed panel layout as a standalone `.eff` file (Effect, Dice & Save, Parameters, Resources, Classification, Caster & Projectile, Resistance), instead of a plain field list - so an effect reads the same whether it stands alone or is embedded in a creature.
