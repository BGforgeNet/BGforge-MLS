...
- You can now add effects to an item (`.itm`) or spell (`.spl`) directly. The Effects tab has an "+ add" button that appends a new global effect (one belonging to no ability), so even an item with no effects can gain its first one. Each ability also has an "Add effect" action that adds an effect owned by that ability, and a newly added ability starts with one effect ready to edit.
