...
- The binary editor now flags cross-record inconsistencies. A creature spell-memorization or item-slot entry that points past its list, or an item/spell ability (or equipping/casting effect range) that runs past the effects list, is shown as a warning with a one-click fix to bring it back in range. Two memorization ranges (creature) or ability/effect ranges (item/spell) that claim the same entry are also warned. Items used by no slot, and effects covered by no ability, are noted for information only - nothing is changed automatically.
- A creature's (`.cre`) spells now open as a single spellbook organized by spell type (Priest / Wizard / Innate) and level, instead of three separate lists for known spells, memorization slots, and memorized spells. Each level shows its known spells next to its memorized spells - with their memorized / disabled state and its base / effective slot counts - all editable in place; you can memorize a spell, add or remove known spells, and add a memorization level directly. Memorized spells that no level claims, or that two levels claim, are surfaced in their own list so an inconsistent file is shown in full, with one-click fixes to clamp an over-long range or drop an unassigned spell.
...
- Fixed: a creature's Allegiance (Enemy/Ally), General type, Race, Class, and Alignment dropdowns showed wrong names for many values - a Druid read as "Mage thief", a Sorcerer as "Mage druid", and invented entries appeared (a "Half elf werewolf" race, a "Fighter mage druid cleric" class). These now match the game's IDS tables.
- Effects now read the same across every editor. Effects in a standalone `.eff` file, embedded in a creature (`.cre`, both the modern v2 and older v1 formats), or in an item (`.itm`) / spell (`.spl`) all open with the same dense layout - fields laid out in the order they are stored, in two columns - so an effect looks the same wherever it appears.
...
- A Fallout weapon or armor proto's granted perk now shows as a named dropdown (the Fallout 2 perk list, with "None") instead of a raw number.
...
- An item (`.itm`) or spell (`.spl`) now opens its abilities and effects as a single "Abilities & Effects" tree instead of two separate tabs. Each ability is a row with the effects it owns nested beneath it, and a "Global" group holds the equipping (item) / casting (spell) effects that belong to no ability; the tab shows a combined abilities/effects count. You can add, duplicate, move, and remove both abilities and effects, add an effect to a specific ability or a global one (so an item with none can gain its first), and filter the effects by name. Selecting a row edits that ability or effect in place; a spell ability row also shows its required caster level.
- A Fallout map object's inventory is now editable. Selecting an object (container, critter, and so on) shows an Inventory list in its detail where you can add or remove items; each item is itself a full object you can edit by expanding its row. Previously inventory items could only be edited in place, never added or removed.
