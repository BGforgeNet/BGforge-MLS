...
- **BREAKING (`@bgforge/binary` library, `.pro.json` snapshots):** the canonical document shapes for armor, drug, and critter PROs have been flattened to match the wire format.
    ...
    - Anyone with saved `.pro.json` snapshots for armor, drug, or critter PROs needs to regenerate them (re-save in the binary editor, or pipe the file through `fgbin` again).
