...
- **A package's `src/` never imports the package's own name.** Inside `format/src/`, reach `format-utils` by relative path, not as `@bgforge/format` - the tsconfig `paths` entry, the vitest alias and the bundler all resolve the self-import, so it works while quietly routing internal code through the published entry point. The cost only surfaces once a package has a second entry (`@bgforge/format` vs `@bgforge/format/internal`) and the two can no longer be told apart. `test/` is exempt: a test importing its own package is exercising the surface consumers get. Guarded by `scripts/utils/test/no-package-self-import.test.ts`, which reports the file and specifier.
...
- **BAF diagnostics have two back ends.** `bgforge.weidu.compiler` picks between the WeiDU binary (the reference, and the default) and the extension's own built-in compiler, dispatched from `server/src/weidu-baf/diagnostics.ts` for both the `.baf` and `.tbaf` entry points. The built-in compiles the editor's text directly and needs no WeiDU binary, but it only handles self-contained BAF - it refuses a file using `%variable%` or a `@123` translation reference, since those values only exist once a mod is being installed.
...
- **Milestone close-out commands (scope to what you touched).** `scripts/test-scoped.sh [paths...]` maps changed paths (default: uncommitted git changes) to the affected suites and runs them; `--dry-run` prints the plan. Manually: binary changes -> `pnpm exec vitest run --config binary/vitest.config.ts`; webview/Svelte changes -> the client tests plus the binary-editor render harness. All vitest configs and suites run from any cwd - includes and fixture paths are anchored to their own file, keep it that way. Three tiers, cheapest first: `test-scoped.sh` while iterating, `pnpm test` (~3 min - every category, shallow) when a change outgrows the scoped suites, and `pnpm build:all` + `pnpm test:all` (~14 min) for close-out, or for changes that span subsystems or touch shared build infra, grammars, transpilers, or the server. `pnpm test` covers every category the gate does, but NOT every gate: the coverage thresholds live only in `test:all` (and CI), which is why its unit phase prints `no coverage` in its own name. So `pnpm test` is not a close-out gate however green - a change can pass it and still be committing a threshold breach. Beyond that the two differ only in depth, and a suite that vanishes rather than failing is the hazard to watch: the SSL corpus canary prints the sample size it drew, and `scripts/ensure-weidu.sh` (called by both test scripts, exported as `WEIDU_BIN`) downloads a pinned WeiDU rather than letting the grammar differential skip itself.
