...
**Version management:** Root `package.json` and `server/package.json` must have identical versions (currently 3.6.0). Other packages have independent versions. Bump manually, commit as "Update changelog, bump version: X.Y.Z", then tag.
