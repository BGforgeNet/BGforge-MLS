...
pnpm test              # Dev-loop suite: typecheck, lint, knip, unit tests (no coverage), samples, CLI, smoke, grammars, SSL corpus canary
pnpm test:all          # Close-out gate: the same categories at full depth - test + coverage thresholds + external corpus + integration + the full SSL corpus sweeps + transpile-external. For cross-subsystem / shared-infra close-out; scope to the affected package otherwise (see Important Rules).
