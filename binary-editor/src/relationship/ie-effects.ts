import { opcodeNameDiffers, opcodeNames, opcodeReading } from "@bgforge/binary";
...
/**
 * Effect fields whose meaning the opcode selects, mapped to the slot naming them in an `OpcodeRelationship`.
...
/**
 * The overlay, bound to the engine of the game the session's record came from.
 *
 * `engine` is captured here rather than read off the Model because it belongs to the SESSION, not to a parse:
 * every structure op, undo and JSON load rebuilds the Model, and a field hanging off it would be dropped by
 * each of them. The session holds one relationship model for its lifetime, so binding it here cannot go stale.
 */
export function createIeEffectsModel(engine?: string): RelationshipModel {
    return {
        formatId: "ie-effects",
        fieldOverride(model, node) {
            ...
            if (key === "opcode") {
                ...
                const desc = availabilitySummary(opcodeReading(opcode, engine)?.availability);
                if (desc) override.description = desc;
                // The spec's enum holds the preferred reading's names, which is right until a game says otherwise.
                // Swapped wholesale rather than per value, because the dropdown needs every option; only done when
                // this engine actually reads the current opcode differently, so the common case costs nothing.
                if (engine !== undefined && opcodeNameDiffers(opcode, engine)) {
                    override.enumOptions = opcodeNames(engine);
            ...
            if (key === "resource") {
                ...
                const opcode = siblingValue(model, node, "opcode");
                const type = opcode === undefined ? undefined : opcodeReading(opcode, engine)?.resourceType;
            ...
            const opcode = siblingValue(model, node, "opcode");
            if (opcode === undefined) return;
            const rel = opcodeReading(opcode, engine);
        ...
        dependents(model, editedNode) {
            ...
            const selectsIdsFile =
                editedOpcode !== undefined && opcodeReading(editedOpcode, engine)?.idsFileByParam2 !== undefined;
...
/** The overlay with no game to ask - the BG(2)EE fallback a record opened off disk gets. */
export const ieEffectsModel: RelationshipModel = createIeEffectsModel();
...
// The first two are per engine; the probability check reads only the record, so one instance serves every game.
//
// Memoized per engine, so every format asking for the same engine gets the SAME function - which is both what
// keeps a handful of closures from multiplying across formats and what lets the parity test assert that
// itm/spl/eff share one overlay rather than three lookalikes.
const byEngine = new Map<string, RelationshipModel>();

function sharedModel(engine?: string): RelationshipModel {
    const key = engine ?? "";
    const cached = byEngine.get(key);
    if (cached !== undefined) return cached;
    const model = createIeEffectsModel(engine);
    byEngine.set(key, model);
    return model;
}

export const ieEffectsFieldOverride = (engine?: string): RelationshipModel["fieldOverride"] =>
    sharedModel(engine).fieldOverride;
export const ieEffectsDependents = (engine?: string): RelationshipModel["dependents"] => sharedModel(engine).dependents;
