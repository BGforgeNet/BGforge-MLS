...
| Timed delay         | `procedure p in <expr>` requires an integer. The delay is one fixed 32-bit field of the procedure table - no type tag, and no code behind it to evaluate an expression - so anything else can only land there as whatever number it reduced to. The reference accepts one and emits a number anyway.                    |
