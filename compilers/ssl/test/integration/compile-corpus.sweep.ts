 ...
 *
 * The gate is that NOTHING with an oracle comes back different. Every script the bundled compiler refused
 * is excluded by name against a pinned list, because an exclusion nobody can see is a comparison that
 * quietly shrank.
 */
...
export function registerCompileShard(index: number, count: number): void {
    ...
    describe.skipIf(!ready)(`real corpus compiles to matching bytecode (${index}/${count})`, () => {
        ...
        beforeAll(async () => {
            ...
            for (const script of mine) {
                ...
                if (digest === undefined) {
                    // A script the manifest has never seen means the corpus and manifest disagree about
                    // the population itself - regeneration territory, counted rather than skipped.
                    unlisted++;
                    continue;
                }
                if (digest === "refused") {
                ...
                try {
                    ...
                    const why = message.startsWith("unknown ") ? message.replaceAll(/'[^']*'/g, "'X'") : message;
        ...
        it("was generated against the pins as they stand", () => {
            // First, so a red run after a compiler or corpus bump reads as "regenerate", never as a
            // codegen defect. Without this, a compiler bump would leave the sweep green against a dead
            // oracle.
        ...
        it("matches the oracles on the corpus", () => {
            ...
            // The corpus total leads: without it, the oracle count is a number with no population behind
            // it and a shrunken comparison cannot be told from a complete one.
            const summary = [
                `corpus ${scripts.length}, shard ${index}/${count} of ${mine.length}, ` +
                    `oracles ${matching + differing}, matching ${matching}, differing ${differing}, ` +
                    `errors ${errors}, unlisted ${unlisted}`,
            ...
            expect(matching + differing + errors, summary).toBe(mine.length - excludedStems.length);
            // Pinned by name: a refusal outside this set means a script stopped building for a reason
            // nobody has looked at, which silently removes it from the comparison.
            expect(excludedStems.toSorted(), summary).toEqual(expectedExcluded);
