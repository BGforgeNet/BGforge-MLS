...
/** One spawn compiles or decompiles a whole directory, so this is far wider than a parse-check's bound. */
...
describe(`BCS - differential against the reference implementation (${SOURCES.length} sources)`, () => {
    beforeAll(async () => {
        weidu = resolveWeidu();
        await initParser();
        parser = getParser();
