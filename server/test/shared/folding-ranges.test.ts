...
describe("shared/folding-ranges", () => {
    ...
    it("creates folding range for multi-line block node", () => {
        const root = mockNode("program", 0, 5, [mockNode("procedure", 0, 5)]);
        const result = getFoldingRanges(root as never, blockTypes);
    ...
    it("skips single-line nodes", () => {
        const root = mockNode("program", 0, 3, [mockNode("procedure", 2, 2)]);
        const result = getFoldingRanges(root as never, blockTypes);
        expect(result).toEqual([]);
    });
    ...
    it("handles nested blocks", () => {
        const root = mockNode("program", 0, 10, [
            mockNode("procedure", 0, 10, [mockNode("if_stmt", 2, 5), mockNode("while_stmt", 6, 9)]),
        ]);
        const result = getFoldingRanges(root as never, blockTypes);
    ...
    it("ignores node types not in the foldable set", () => {
        const root = mockNode("program", 0, 5, [mockNode("expression", 0, 5)]);
        const result = getFoldingRanges(root as never, blockTypes);
        expect(result).toEqual([]);
    });

    it("folds multi-line comments with Comment kind", () => {
        const root = mockNode("program", 0, 10, [mockNode("comment", 0, 3)]);
        const result = getFoldingRanges(root as never, blockTypes);
        expect(result).toHaveLength(1);
        expect(result[0]).toEqual({
            startLine: 0,
    ...
    it("folds line_comment as Comment kind", () => {
        const root = mockNode("program", 0, 5, [mockNode("line_comment", 1, 3)]);
        const result = getFoldingRanges(root as never, blockTypes);
        expect(result).toHaveLength(1);
        expect(result[0]).toEqual({
            startLine: 1,
    ...
    it("skips single-line comments", () => {
        const root = mockNode("program", 0, 5, [mockNode("comment", 2, 2)]);
        const result = getFoldingRanges(root as never, blockTypes);
        expect(result).toEqual([]);
    });
    ...
    it("handles deeply nested structures", () => {
        const root = mockNode("program", 0, 20, [
            mockNode("procedure", 0, 20, [mockNode("if_stmt", 1, 15, [mockNode("while_stmt", 3, 10)])]),
        ]);
        const result = getFoldingRanges(root as never, blockTypes);
        expect(result).toHaveLength(3);
    ...
    it("finds foldable nodes nested inside non-foldable parents", () => {
        const root = mockNode("program", 0, 15, [
            mockNode("expression", 0, 12, [mockNode("procedure", 2, 8), mockNode("if_stmt", 9, 11)]),
        ]);
        const result = getFoldingRanges(root as never, blockTypes);
    ...
    it("works with empty blockTypes set", () => {
        const root = mockNode("program", 0, 5, [mockNode("procedure", 0, 5), mockNode("comment", 6, 8)]);
