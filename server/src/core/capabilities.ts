...
export interface ProviderContext {
     ...
     * workspace - most of startup on a large mod - and it runs on the same thread as everything else, so
     * anything started beside it finishes only when it does. Holding it until the translation load has
     * landed keeps `@N`/message previews at their pre-background timing instead of pushing them behind
     * the parse.
     */
