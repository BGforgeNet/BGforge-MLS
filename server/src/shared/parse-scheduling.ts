 ...
 * everywhere - a real WeiDU installer runs to hundreds of kilobytes, where the same parse holds the
 * server's event loop on EVERY keystroke, delaying every other request queued behind it. On
 * `external/infinity-engine/BGT-WeiDU/bgt/bgt.tp2` (443 KB, 12250 lines) the mean parse
 * (`server/test/perf/tp2-parse.bench.ts`) runs past the server's own 50 ms per-request budget.
 *
 ...
 *
 * Set well below the installer measured above and well above any ordinary script (the largest committed
 * grammar sample is 23 KB), so in practice only real installers debounce.
 */
