...
export async function buildConsumerIndex(state: TranslationState): Promise<void> {
    ...
    // workspace both globs return the identical file set. Left duplicated on purpose: the walk is around a
    // hundredth of startup, and sharing one list means exposing the registry's extension union and
    // reordering onInitialize, which is a reachability change for that same fraction. The parsing inside
    // that scan is where the time actually is.
