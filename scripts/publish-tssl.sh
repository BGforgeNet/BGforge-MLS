...
# Build and publish the @bgforge/tssl package (compiler library + tssl bin) to npm.
# Usage: ./scripts/publish-tssl.sh [--dry-run]
...
if [ "${SKIP_BUILD:-}" != "1" ]; then
    echo "=== Building @bgforge/tssl ==="
    pnpm build:tssl
fi

do_publish "@bgforge/tssl" compilers/tssl "$@"
