...
| `pnpm test:all`      | Close-out gate: adds coverage, external corpus, integration, the full sweeps  |
...
- **External-corpus format sweep** (`test-external.sh`) -- multi-minute, and it MUTATES the external trees
  (restored by its own EXIT trap), so it cannot share a parallel block with anything else reading them.
- **Server integration** (`server/test/integration/`) -- ~70s; included by `pnpm test:all`.
...
| Script                               | Description                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `test.sh`                            | Dev-loop test suite run by `pnpm test`. Typechecks client/server/plugins/binary/format/transpilers, runs Oxlint, unit tests (no coverage), TD/TBAF sample tests, formatting checks, CLI tests, knip, the grammar suites, and the SSL corpus canary - all in parallel phases. `TEST_COVERAGE=1` switches the unit phase to coverage runs with thresholds; `TEST_STOP_AFTER_BUILD=1` exits after Phase 2 (both used by `test-all.sh`). |
