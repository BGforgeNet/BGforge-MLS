...
- **One vitest process aggregating every project** (the root `vitest.config.ts` shape) instead of the separate
  per-project runs `test.sh` starts -- no better than the per-suite `--maxWorkers` caps, so the extra coupling buys
  nothing.
...
| `test-coverage.sh`                    | Coverage for the packages a change touched, using each package's own vitest thresholds. Usage: `pnpm test:cov [packages...]`; with no argument it covers every package that has a config.                                                                                                                                                                                                                                                                |
| `ensure-weidu.sh`                     | Print a WeiDU binary's path, downloading a pinned checksum-verified one into `.dev/` when the host has none. Called by `test.sh`, `test-all.sh` (which export it as `WEIDU_BIN`) and the CI build workflow, so every WeiDU-backed suite reuses one resolved binary instead of provisioning its own.                                                                                                                                                      |
...
| `build-packages.sh`                   | Build every package bundle, in parallel. Needs a prior `pnpm build:grammar`, because the format CLI's post-build step copies the grammar WASMs into `format/out`.                                                                                                                                                                                                                                                                                        |
...
| `build-ssl-postbuild.sh`              | Post-build hook for `@bgforge/ssl`: copies the tree-sitter WASM files next to `compilers/ssl/out/cli.js`, which resolves them via `__dirname`. Invoked by tsdown's `onSuccess` hook.                                                                                                                                                                                                                                                                     |
...
| `fallout-update.sh`                   | Update Fallout SSL data from a pinned sfall checkout in `external/sfall-data`. Writes `server/data/fallout-ssl-sfall.yml`, then calls `generate-data.sh` and `syntaxes-to-json.sh` unless `MLS_SKIP_REGEN=1` is set.                                                                                                                                                                                                                                     |
...
| `verify-vsix-runtime-deps.mjs`        | Check that every runtime dependency the packaged server declares resolves from inside the VSIX, that dependency's own dependencies included.                                                                                                                                                                                                                                                                                                             |
...
| `esbuild-elk-worker.mjs`              | Shared esbuild plugin embedding elkjs's layout worker. Used by `build-webviews.mjs` and the dialog render harness build, so both deliver the worker the same way.                                                                                                                                                                                                                                                                                        |
| `esbuild-svelte-warnings.mjs`         | Shared esbuild-svelte warning filter, dropping Svelte compiler warnings raised inside `node_modules` sources so warnings from this repo's own components stay visible.                                                                                                                                                                                                                                                                                   |
...
| `bench-thumbnails.mts`                | On-demand breakdown of where thumbnail time goes, per stage and per variant. Run with `pnpm bench:thumbnails`; no gate runs it.                                                                                                                                                                                                                                                                                                                          |
