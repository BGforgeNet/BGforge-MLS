...
| `pnpm test`          | Dev-loop test suite from `test.sh` (no coverage, external, or integration)    |
| `pnpm test:all`      | Close-out gate: adds coverage, external corpus, integration, grammar suites   |
...
- **External corpus + integration** (`test-external.sh`, `server/test/integration/`) -- multi-minute tail; included by `pnpm test:all`.
...
- **Integration tests** (`test/integration/`) -- require external repos cloned via `pnpm test:external`. Run standalone with `cd server && pnpm test:integration`, or as part of `pnpm test:all` (which clones repos first).
...
| Script                               | Description                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `test.sh`                            | Dev-loop test suite run by `pnpm test`. Typechecks client/server/plugins/binary/format/transpilers, runs Oxlint, unit tests (no coverage), TD/TBAF sample tests, formatting checks, CLI tests, and knip - all in parallel phases. `TEST_COVERAGE=1` switches the unit phase to coverage runs with thresholds; `TEST_STOP_AFTER_BUILD=1` exits after Phase 2 (both used by `test-all.sh`). |
...
| `test-external.sh`                   | Clone external repos and run format/idempotency tests against them. Also provides fixtures for integration tests. Skips the pre-test repo reset when `EXTERNAL_REPOS_CLEAN=1` (set by `test-all.sh`).                                                                                                                                                                                     |
...
| `grammar-generate-lib.sh`            | Cached `tree-sitter generate` wrapper (skips regeneration when grammar inputs are unchanged). Sourced, not executed.                                                                                                                                                                                                                                                                      |
...
| `test-all.sh`                        | Full test suite run by `pnpm test:all`: main suite phases 1-2, then grammar tests and the remaining suites (smoke, samples, external, integration, transpile-external) in one parallel block.                                                                                                                                                                                             |
