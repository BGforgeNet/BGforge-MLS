...
- `render-layout-thread.mts` - the layout-thread driver: asserts the graph layout runs in a real Worker, that
  re-laying out a large dialog never blocks the main thread for long, and that the webview reports the blocks
  it does have to the host. Browser-only - node has neither `Worker` nor a long-task entry type, so
  `dialog-layout.test.ts` exercises the inline fallback and `webview-utils.test.ts` a stand-in observer;
