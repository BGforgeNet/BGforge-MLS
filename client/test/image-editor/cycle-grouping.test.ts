 ...
 */
function baseFileSequences(blocks: number): { sequences: SequenceView[]; frameCount: number } {
...
describe("interpretIeRose", () => {
    ...
    test("east slots stuffed with DIFFERENT constant frames are not one shared filler", () => {
        ...
        expect(interpretIeRose(sequences, frameCount)?.detected).toBe(false);
    });
    ...
    test("an E-file shape (west slots empty, east slots real) is interpretable but not detected", () => {
        ...
        const result = interpretIeRose(sequences, 7);
        expect(result?.detected).toBe(false);
    ...
    test("a lone <=8-cycle set maps slots to the IE order without claiming detection", () => {
        ...
        const result = interpretIeRose(sequences, 5);
        expect(result?.detected).toBe(false);
    ...
    test("returns undefined for shapes that cannot map onto 8-slot blocks", () => {
        ...
        expect(interpretIeRose([seq([0], "NE")], 1)).toBeUndefined();
        expect(interpretIeRose([], 0)).toBeUndefined();
    });

    test("a block with no real west cycle defeats detection (nothing to show on the rose)", () => {
        const { sequences, frameCount } = baseFileSequences(2);
