...
test("accepts valid messages", () => {
    ...
    expect(isWebviewToHost({ type: "import", kind: "png-directory", mode: "append" })).toBe(true);
});
test("rejects malformed messages", () => {
    ...
    expect(isWebviewToHost({ type: "import", kind: "png-directory", mode: "wrong" })).toBe(false);
});
